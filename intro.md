...
map' 3 [1,2,3]    --=>[4,5,6]
```
做了如上改进，我们让`map'`传入两个参数，一个是加运算的操作数另一个是被操作的列表，其中`map`就是一个容器，`(+x)`就是容器中的函数。接下来在`map'`得到参数`x=3`时，就变成了`map (+3) list`，我们再把`(+3)`用匿名函数的方式展开就变成了：`map (\x->x+3) list`，这时我们看到`3`变成了`(\x->x+3)`函数的一部分，而`3`本身不是函数中定义的，是通过容器`map``传入的。这种函数自由访问容器中外部变量的特性就是闭包，整个容器就是这个函数和它外部变量的`闭包`。
