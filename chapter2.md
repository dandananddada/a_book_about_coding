...
* Java中不允许数字类型与布尔类型间转换。
* Boolean.parseBoolean会将字符串true（不区分大小写）转换为布尔值true，而其他字符串都会转换为false。
...
这样0.625表示为二进制就是001。
对于小数的存储，常见的方式有32位和64位两种，在32位中把第32位作为符号位，第31位到23位这8位作为小数位，剩下的23位用来表示整数
...
* 对于需要N个字节来存储的符号，第一个字节的前N位置为1，第N+1位置为0；后面N-1个字节的前两位都是10。比如三个字节存储的字符对应为:`1110xxxx 10xxxxxx 10xxxxxx`
