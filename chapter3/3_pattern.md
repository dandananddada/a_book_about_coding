...
设计模式本身是一种规范与技巧，其存在的目的主要是完善或者弥补面向对象语言在编程开发上的补足。一些语言无法更好解决的问题，开发中可以通过适当的使用设计模式来妥善的处理，但是如果语言本身具备了你在开发中所需要的特性，那么设计模式就开抛到一边了。就像Java提供了C++没有的垃圾回收机制，你就无需在Java中手动释放内存了，Scala提供了Java没有的线程管理机制，你就无需在Scala中加线程锁一样。如果语言本身提供了更好的特性，你也就不用编写多余的接口实现类来实现各种设计模式中所描述的思路了。
