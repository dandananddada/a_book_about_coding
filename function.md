...
在Java8中加入了lambda特性，多数语言没有提供匿名函数但是都提供了lambda表达式，使用上lambda和匿名函数所产生的效果是一致的，但是其实两者只是存在功能上的交集，并非相同概念（lambda定义的函数可以是署名的，但lambda表达式定义函数只作为数据使用，而匿名函数是可以直接执行的，比如JavaScript的那个例子）。
