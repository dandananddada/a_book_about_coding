...
其中前三行是断言描述了\`a=b\`，\`c=b\`， \`b=d\`。后一行是推断：\`a = c的前提条件是：a = b且 b = c\`。
...
我们以第一个查询\`equalToo\(a,c\).\`为例说明，带入推断后得到：\`equal\(a, Z\), equal\(c, Z\)\`。对比断言我们可以求出\`Z=b\`，也就是如果\`Z=b\`则断言运算后返回\`true\`，因此第一个查询返回\`yes\`。第二个查询同理，因为程序运算后找不到符号条件的
